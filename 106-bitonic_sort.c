...
void bitonic_merge(int *array, size_t start, size_t seq, char dir)
{
	...
	if (seq > 1)
	{
		...
		bitonic_merge(array, start, jump, dir);
		bitonic_merge(array, start + jump, jump, dir);
...
void bitonic_seq(int *array, size_t start, size_t seq, char dir)
{
	...
	if (seq > 1)
	{
		bitonic_seq(array, start, cut, UP);
		bitonic_seq(array, start + cut, cut, DOWN);
